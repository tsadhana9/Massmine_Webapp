...
def validate_massmine(request):
	...
	child = pexpect.spawn('/usr/bin/massmine/./massmine --task=twitter-auth')
	child.expect('[No]')
	child.sendline('yes')
...
def validate_massmine_facebook(request):
	...
	child1 = pexpect.spawn('/usr/bin/massmine/./massmine --task=facebook-auth')
	child1.expect('[No]')
	child1.sendline('yes')
...
def validate_massmine_instagram(request):
	...
	child2 = pexpect.spawn('/usr/bin/massmine/./massmine --task=instagram-auth')
	child2.expect('[No]')
	child2.sendline('yes')
...
def validate_massmine_youtube(request):
	...
	child3 = pexpect.spawn('/usr/bin/massmine/./massmine --task=youtube-auth')
	child3.expect('[No]')
	child3.sendline('yes')
...
def make_query(request):
	...
	if (validate_massmine(request) == 1):
		...
		command = '/usr/bin/massmine/./massmine --task=twitter-stream --geo=' + str(p3) + ' --count=' + '"' + count + '"' + ' --query=' + '"' + keyword + '"' + '"' + mydropdown1 + '"' + keyword2 + '"' + mydropdown2 + '"' + keyword3 + '"' + str(from_1) + '"' + str(to_1)
